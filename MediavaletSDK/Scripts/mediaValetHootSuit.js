...
+(function (source, $) {
    ...
    $(queryDict).each(function (index, data) {
        hootsuiteuserid = data.uid;
    });
    var mediaValetHootsuite = function () {
        /**function Initialization
        */
        mediaValetHootsuite.prototype.Initialization = function () {
            hsp.init({
        ...
        *@param imageurl
        */
        mediaValetHootsuite.prototype.insertAsset = function (imageurl) {
            $('#statusContainer > .info').css('background-color', '#000 !important');
            var filenameandext = mediaValetOffice.prototype.filenameandext(imageurl);
            try {
                if (imageurl === "") {
                } else {
                    var seconds = new Date().getTime() / 1000;
                    seconds = parseInt(seconds);
                    var timestamp = sha512(hootsuiteuserid + seconds + imageurl + 'SharedSecret');
                ...
                hsp.showStatusMessage('Preparing your Message ', 'info');
                
            } catch (e) {
            }
        }
        /**function filenameandext
        *@param imageurl
        */
        mediaValetHootsuite.prototype.filenameandext = function (imageurl) {
            var _filename = '';
            var fileExtension = imageurl.split('/');
            var len = fileExtension.length;
            _filename = fileExtension[len - 1];
            return _filename.split('.');
        }
        /**Function ApplicationInsights
      *@param instrumentationkey 
      */
        mediaValetHootsuite.prototype.ApplicationInsights = function (instrumentationkey) {
    ...
    source.mvAppSdkCore = new mediaValetHootsuite();
