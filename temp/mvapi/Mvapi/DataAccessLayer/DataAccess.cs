...
namespace Mvapi.DataAccessLayer
{
    public class DataAccess
    {
        ...
        public static MediaValetCategoryDataEntity GetCategoryData(string orgunitid, string application)  //string domain, string email, 
        {
            ...
            string orgidFilter = TableQuery.GenerateFilterCondition("OrgUnitId", QueryComparisons.Equal, orgunitid);
            string appFilter = TableQuery.GenerateFilterCondition("Application", QueryComparisons.Equal, application);
            tableQuery.FilterString = TableQuery.CombineFilters(orgidFilter, TableOperators.And, appFilter);
        ...
        //for domain mapping
        public static MediaValetDomainDataEntity GetDomainData(string SearchVal,string SearchField)
        {
            //var domainName = Domain.Split('@');
            CloudTableClient tableClient = account.CreateCloudTableClient();
            var tableName = ConfigurationManager.AppSettings["DomainTableName"];
            TableQuery<MediaValetDomainDataEntity> query=null;
            CloudTable table = tableClient.GetTableReference(tableName);
            if (SearchField == "email")
            {
                query = new TableQuery<MediaValetDomainDataEntity>().Where(TableQuery.GenerateFilterCondition("EmailDomain",
                                                                QueryComparisons.Equal, SearchVal));
            }
            else if (SearchField == "domain")
            {
                query = new TableQuery<MediaValetDomainDataEntity>().Where(TableQuery.GenerateFilterCondition("DomainName",
                                                                QueryComparisons.Equal, SearchVal));
            }
        ...
        public static string SaveDomainData(string domainName, string apiUrl, string email)
        {
            try
            {
                CloudTableClient tableClient = account.CreateCloudTableClient();
                ...
                tableQuery.FilterString = TableQuery.CombineFilters(
                        ...
                        TableOperators.And,
                        TableQuery.GenerateFilterCondition("EmailDomain", QueryComparisons.Equal, email));
                        
                var data = table.ExecuteQuery(tableQuery);
                if (data.FirstOrDefault() == null)
                {
                    MediaValetDomainDataEntity entity = new MediaValetDomainDataEntity(domainName, apiUrl, email);
        ...
        public static MediaValetMetaData GetMetaData(string email, string domaniname, string orgunitid, string appsname)
        {
            ...
            var query = from mvdata in tableQuery.CreateQuery<MediaValetMetaData>()
                        where mvdata.DomainName == domaniname && mvdata.Email == email && mvdata.OrgUnitId == orgunitid && mvdata.Application == appsname
                        select mvdata;
        ...
        public static string AddMetaData(MetaDataModel model)
        {
            ...
            try
            {
                ...
                var querys = from mvdata in tableQuery.CreateQuery<MediaValetMetaData>()
                            where mvdata.DomainName == model.DomainName && mvdata.Email == model.Email && mvdata.OrgUnitId == model.OrgUnitId && mvdata.Application == model.Application
                            select mvdata;
        ...
        public static string EloquaAssetTracking(EloquaTrackingModel eloquaTrackingModel)
        {
            try
            {
                ...
                if (querys.FirstOrDefault() == null)
                {
                    EloquaTrackingEntity entity = new EloquaTrackingEntity(eloquaTrackingModel.instanceid, eloquaTrackingModel.customerid, eloquaTrackingModel.imageurl, eloquaTrackingModel.assetid, eloquaTrackingModel.assetname, eloquaTrackingModel.assettype, eloquaTrackingModel.sasurl);
